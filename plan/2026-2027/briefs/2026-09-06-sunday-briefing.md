---
mode: A (§16 Monday/Sunday briefing)
to: avishag@apex.org.il
subject: בריפינג שבועי — אנשים · 6.9
built: 2026-09-01
status: Gmail draft r8768271639676627333, created 2026-09-01, updated 2026-09-01 (rev. 2: Anne Neuberger, Avichai Radovsky/Jackie Levi, David Shasha 3.9 call)
sources:
  - correspondence sweep, both addresses, newer_than:7d (29 threads)
  - plan/2026-2027/call-schedule.csv, week 1
  - config/network/roster.csv
  - calendar, 6–10 Sep
  - Dolev Elbaz WhatsApp export, 16.8
  - two user-reported calls (Matan Shoef, Nadav Cohen) — no date given
---

היי אבישג,

לפני הישיבה ביום שני. הכל כאן מגיע מההתכתבות בינינו, מוואטסאפ עם דולב,
ומהרוטציה שבניתי — לא מהזיכרון.

**Top People**

דולב אלבז | Cohort 3 | פירט בוואטסאפ (16.8) ארבע הצעות: שני עמיתים
למחזור/יחידה כמועמדי Contributor, חיבור למשקיע שלו F2, וחבר עם דוקטורט
ב-vision מאפל כמרצה פוטנציאלי | להחליט על מי מהארבע רוצים לקדם | נדב מטפל,
ממתין לתשובתך

תמיר מירוביץ׳ | לא בוגר — קהילה | ← ממך, 1.9: מכיר הרבה אנשים ועוזר בחיבורים
כשמפקסים אותו, מכפיל כוח | להבין ממך איך מפקסים אותו | ביחד, בישיבה ב-7.9

מתן שואף | Cohort 3, Tier B | שוחחתי איתו — הצעתי חיבור ל-Pearl והתייעצות |
לוודא שהחיבור בוצע | נדב מטפל

נדב כהן | Cohort 3, Tier C | שוחחתי איתו — הציע שיתוף פעולה עם הקרן Deep
Insight שהוא עובד בה. זה גם סוגר ספק זיהוי שהיה פתוח אצלי לגביו | לגבש הצעה
קונקרטית | נדב מטפל

דוד שאשא | Cohort 3, Tier C | קבענו שיחה ל-3.9 (יום חמישי) על שותפות
טווח ארוך | לוודא שהשיחה התקיימה ולעדכן אותך בתוצאה | נדב מטפל

**Opportunities**

1. **ארבע ההצעות של דולב.** שני מועמדי Contributor (רון זיידמן — VP R&D
   פעמיים, ex-Microsoft; ישראל סופר — רמ״ד ב-81, כשנה לשחרור), חיבור למשקיע
   שלו F2, וחבר עם דוקטורט ב-vision מאפל למרצה. למה עכשיו: יעד הרבעון הוא 8
   Contributors ל-Office Hours ועדיין אין אף מועמד סגור — וההצעה פתוחה
   מדולב, Tier A, מ-16.8.
2. **חיבור לז׳קי לוי, מטה הבינה המלאכותית הלאומי.** אביחי רדובסקי (Cohort 3
   — ותוך כדי זה גילינו שהוא ארכיטקט, לא "Unknown" כמו שהיה רשום) רוצה לחבר
   בינינו, בהקשר frontier labs. למה עכשיו: קשר ברמת המטה הלאומי הוא נדיר
   ואביחי מציע אותו באופן פעיל.
3. **SF.** יותם ניסים ו-Zack נחתו בארץ (שלחתי לך 31.8). למה עכשיו: את
   ב-Silicon Valley 9–15.10, ופאזות 1–2 של SF הן למפות ולפגוש — זום איתם
   לפני הנסיעה הופך אותם לצומת ולא לעוד שם ברשימה.

**Risks / Weak Signals**

1. **שלושה מיילים ממני אלייך בלי תשובה.** מתנדבים — דוד שאשא, נעמה (27.8), SF AI (31.8),
   פאונדר Inception (31.8).
2. **Technion x Apex ב-6.9.** ביקשת ב-27.8 להזיז ל-14:20, ההזמנה עדיין על
   14:30. סוגר מול נדב קמפפר.
3. **שתי שיחות בלי תאריך מדויק.** מתן ונדב כהן — שתיהן קרו, אבל בלי
   last_contact אמיתי הרוטציה וזיהוי "מי לא דיברנו איתו מזמן" לא עובדים.
   מבקש מעצמי ומכל מי שמדבר עם אפקס לתעד תאריך תמיד, גם בהודעה קצרה.

**מה אני צריך ממך** — שניים

1. איפה את מתעדת שיחות עם אפקסים | למה עכשיו: מתחיל 4 שיחות בשבוע ב-6.9
   ואנחנו נעשה כפילויות | למה דווקא את: רק את יודעת איפה ה-State הזה יושב
   היום | מה בדיוק: לענות איפה, ואם זה לא במקום אחד — להחליט שמהיום זה
   הרוסטר | זמן נדרש: 2 דקות
2. איזה מספר הוא ה-denominator שלנו | למה עכשיו: כל אחוז ב-KPI-ים תלוי בו |
   למה דווקא את: זו החלטה שלך מה נספר כבוגר | מה בדיוק: לבחור אחד — 97, 117
   (מה ששלחתי לך ב-26.8) או 82 (ה-Canon) | זמן נדרש: דקה

**השבוע**

*שיחות בוגרים — שבוע 1 של הרוטציה*

- ראשון 6.9 · אסף מונסה | Tier A — מועמד Node. מה הוא יכול לפתוח לבוגרים
  שלנו, והאם ייתן Office Hours
- שני 7.9 · דני גרנדר | Tier A — ייסד את Snyk. אותה שאלה, המועמד הכי חזק
  להיות צומת
- שלישי 8.9 · אלדד קפטן | Tier B — מה הוא יכול לתת, ומה הוא צריך מאיתנו
  עכשיו
- רביעי 9.9 · אביאל אדלר | Tier C — Roll-Up: על מה הוא עובד היום ומה יעזור
  לו
- רביעי 9.9 · מתן שואף | Tier B — כבר ביומן ב-11:00

*פגישות*

6.9 14:30 Technion x Apex (בכפוף להזזה ל-14:20) · 7.9 15:00 הישיבה שלנו ·
8.9 11:00 CJP <> APEX · 8.9 17:00 Yoav <> Apex

*Action items פתוחים*

- לענות לדולב אם רוצים את החיבורים לרון, ישראל, F2 והמרצה
- לתאם עם אביחי מתי ואיך מתחברים לז׳קי לוי
- לבדוק את השאלון שהעברת ב-31.8
- לסגור את Technion x Apex ל-14:20 מול נדב קמפפר
- להירשם ל-The New Pioneers ב-8.9
- סיכום מכל שיחה נכנס לרוסטר תוך 24 שעות, עם תאריך אמיתי

---

**פרופילים נוספים מההתכתבות — לא בוגרי אפקס, ליידע בלבד**

מהמיילים האחרונים, לא לפעולה מיידית:

- עדן — פאונדר שפגשתי באירוע Lux, שלב Inception [[NADAV: שם משפחה?]]
- יותם ניסים — נחת בארץ, worth a Zoom
- Zack — אותו הקשר
- Anne Neuberger — ביקשת להוסיף, לפגוש באירוע Pioneering
  [[NADAV: זה אותו "New Pioneers" ב-8.9?]]

בישיבה ביום שני הייתי רוצה שנשרוף את הזמן על תמיר, על החיבור לז׳קי לוי ועל
ה-denominator — כל השאר כאן ולא צריך לעבור עליו.

דלג׳ו

---

## Open items this revision surfaced (for Nadav, not in the mail)

- **Ron Zaidman** and **Israel Sofer** (Dolev's referrals) are not in
  `roster.csv`. Per CLAUDE.md — "never add a person unasked" — they were kept
  out of the roster and only named in the drafts. Proposed rows, if wanted:
  - Ron Zaidman · same cohort as Dolev (#3) · VP R&D (second such role),
    ex-Microsoft, indie "Dopamine" agents project · linkedin.com/in/ron-zaidman
    · +972525972552
  - Israel Sofer · a later cohort than Dolev's · רמ״ד in a core unit, ~1yr to
    discharge · linkedin.com/in/israel-sofer · +972549256346
- **Gal Weiner** and **Or Wilder** (the other two GB300-Princeton connections)
  are also not in the roster — need confirming as Apex alumni before adding.
- **Liran Merkin vs Liran Markin** — the paste spelled it "מרקין"; the existing
  roster row is "Liran Markin" (Cohort 2, CEO at Edwin). Treated as the same
  person; flagged in the row's notes for confirmation.
- **Matan Shoef / Nadav Cohen call dates** — not given. `last_contact` was left
  blank rather than guessed.
- **Avichay Radovsky's role** updated from "Unknown" to "Architect" — roll-up
  progress, told directly by Nadav.
- **Calendar-suggestion Google Tasks created** (list: Velocity + Apex):
  "📅 David Shasha, Thu 3.9 — add to calendar?" and "📅 Anne Neuberger, אירוע
  Pioneering — add to calendar?" (date left open — flagged as an assumption
  this is the same "New Pioneers" event from 27.8). A third task — coordinate
  the Avichai Radovsky → Jackie Levi intro — **failed**: Zapier account hit
  its task quota ("insufficient tasks... visit zapier.com/pricing").
